...
- Added application-level ACL integration for ClamAV.

- Added check-first ACL handling to avoid unnecessary repeated ACL changes.

- Added targeted `clamav` ACL entries to application directories created by `create-php-app.sh`.

- Added default ACL inheritance for runtime writable directories.

- Added ClamAV integration through `clamonacc` for on-access malware scanning.

- Added recursive ClamAV monitoring for `/var/apps`.

- Added `--fdpass` support to the `clamonacc` service configuration to support on-access scanning with the ClamAV daemon.

- Added ClamAV EICAR end-to-end upload testing through the CSIRT Lab test environment.

- Added Linux Malware Detect (LMD) installation and integration.

- Added YARA installation and integration with LMD.

- Added custom YARA signature support through the LMD custom signature directory.

- Added LMD YARA scanning configuration for application directories.

- Added LMD quarantine handling for detected malware.

- Added quarantine path alias:

  - `/usr/local/maldetect/quarantine`

  - `/opt/quarantine`

- Added `test-csirtlab.sh` for automated fresh-VM validation of the PHP application deployment and malware-protection stack.

### Security

- ClamAV provides the on-access malware detection layer for files entering `/var/apps`.

- LMD provides scheduled/deep malware scanning of `/var/apps`.

- YARA is integrated into LMD for rule-based malware detection.

- LMD quarantine is enabled for confirmed malware hits.

- ClamAV ACL access is limited to the minimum directory permissions required for scanning:

  - `user:clamav:r-x` on relevant application directories.

  - `default:user:clamav:r-x` on runtime directories requiring inherited ACLs.

  - `user:clamav:--x` on `/var/apps` where only directory traversal is required.

- The project does not use recursive `setfacl -R` over the entire application tree for ClamAV ACL configuration.

- Writable application directories remain separated from read-only application source code.

- Malware scanning is performed independently from the application upload logic; the application does not need to invoke ClamAV directly for on-access detection.

### Testing

- Verified ClamAV `clamonacc` monitoring of `/var/apps` and all subdirectories.

- Verified ClamAV on-access detection of the EICAR test signature through a web upload.

- Verified LMD + YARA detection and quarantine using a custom test signature.

- Verified ACL consistency across applications generated by `create-php-app.sh`.

- Verified the generated `legacy-app` and `legacy-app1` application directories receive the expected ClamAV ACL configuration.

- Verified the CSIRT Lab environment after VM restoration.
...
\| ****1.2.0**** | ****2026-09-22**** | Added ClamAV on-access malware protection, ClamAV ACL integration, Linux Malware Detect (LMD), YARA integration, malware quarantine handling, and CSIRT Lab end-to-end security testing. |
...
[Unreleased]: https://github.com/NRTechnology/docker-php/compare/v1.2.0...HEAD
...
[1.2.0]: https://github.com/NRTechnology/docker-php/releases/tag/v1.2.0
